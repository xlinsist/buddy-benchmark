#map = affine_map<(d0) -> (d0 ceildiv 1)>
            ...
            affine.for %kernel_col_idx = %c0 to %kernel_col {
              affine.for %output_col_idx = %c0 to #map(%output_col) {
                ...
                scf.if %sparsity_flag {
                  // Check tail.
                  %kernel_vec = vector.broadcast %kernel_ele : f32 to vector<1xf32>
                  ...
                  scf.if %tail_flag {
                    %input_vec = affine.vector_load %input[%n_idx, %c_idx, %output_row_idx + %kernel_row_idx, %kernel_col_idx + %output_col_idx * 1] : memref<?x?x?x?xf32>, vector<1xf32>
                    %output_vec = affine.vector_load %output[%n_idx, %f_idx, %output_row_idx, %output_col_idx * 1] : memref<?x?x?x?xf32>, vector<1xf32>
                    %result_vec = vector.fma %input_vec, %kernel_vec, %output_vec : vector<1xf32>
                    affine.vector_store %result_vec, %output[%n_idx, %f_idx, %output_row_idx, %output_col_idx * 1] : memref<?x?x?x?xf32>, vector<1xf32>
                  } else {
                    ...
                    %input_vec_tail = vector.maskedload %input[%n_idx, %c_idx, %input_row_idx_tail, %input_col_idx_tail], %mask_vec, %c0_f32_vec : memref<?x?x?x?xf32>, vector<1xi1>, vector<1xf32> into vector<1xf32>
                    %output_vec_tail = vector.maskedload %output[%n_idx, %f_idx, %output_row_idx, %output_col_idx_tail], %mask_vec, %c0_f32_vec : memref<?x?x?x?xf32>, vector<1xi1>, vector<1xf32> into vector<1xf32>
                    %result_vec_tail = vector.fma %input_vec_tail, %kernel_vec, %output_vec_tail : vector<1xf32>
                    vector.maskedstore %output[%n_idx, %f_idx, %output_row_idx, %output_col_idx_tail], %mask_vec, %result_vec_tail : memref<?x?x?x?xf32>, vector<1xi1>, vector<1xf32>
                  }
                }
              }
            }
