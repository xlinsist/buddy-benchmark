...
#map3 = affine_map<(d0) -> (d0 ceildiv 1)>

func.func @batch_matmul_broadcast_1(%a : memref<1x64x576xf32>, %b : memref<1x576x3136xf32>, %c : memref<1x64x3136xf32>) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  %step = arith.constant 1 : index
  %c0_f32 = arith.constant 0.0 : f32
  %c0_f32_vec = vector.splat %c0_f32 : vector<1xf32>
  ...
  affine.for %b_row_idx = 0 to %b_row {
    affine.for %a_row_idx = 0 to %a_row {
      affine.for %b_col_idx = 0 to #map3(%b_col) {
        ...
        %a_vec = vector.broadcast %a_ele : f32 to vector<1xf32>
        ...
        scf.if %tail_flag {
          %b_vec = affine.vector_load %b[%c0, %b_row_idx, %b_col_idx * 1] : memref<1x576x3136xf32>, vector<1xf32>
          %c_vec = affine.vector_load %c[%c0, %a_row_idx, %b_col_idx * 1] : memref<1x64x3136xf32>, vector<1xf32>
          %result_vec = vector.fma %a_vec, %b_vec, %c_vec : vector<1xf32>
          affine.vector_store %result_vec, %c[%c0, %a_row_idx, %b_col_idx * 1] : memref<1x64x3136xf32>, vector<1xf32>
        } else {
          %mask_vec = vector.create_mask %tail_len : vector<1xi1>
          %b_col_idx_tail = arith.muli %b_col_idx, %step : index
          %b_vec_tail = vector.maskedload %b[%c0, %b_row_idx, %b_col_idx_tail], %mask_vec, %c0_f32_vec : memref<1x576x3136xf32>, vector<1xi1>, vector<1xf32> into vector<1xf32>
          %c_vec_tail = vector.maskedload %c[%c0, %a_row_idx, %b_col_idx_tail], %mask_vec, %c0_f32_vec : memref<1x64x3136xf32>, vector<1xi1>, vector<1xf32> into vector<1xf32>
          %result_vec_tail = vector.fma %a_vec, %b_vec_tail, %c_vec_tail : vector<1xf32>
          vector.maskedstore %c[%c0, %a_row_idx, %b_col_idx_tail], %mask_vec, %result_vec_tail : memref<1x64x3136xf32>, vector<1xi1>, vector<1xf32>
        }
      }
    }
  }
...
func.func @conv2d_nchw_fchw_im2col(%input: memref<?x?x?x?xf32>, %kernel: memref<?x?x?x?xf32>, %output: memref<?x?x?x?xf32>) {
  ...
  func.call @batch_matmul_broadcast_1(%kernel_expand, %input_collapse, %output_collapse) : (memref<1x64x576xf32>, memref<1x576x3136xf32>, memref<1x64x3136xf32>) -> ()
  // Apply col2im.
